...
function App() {
  ...
  useEffect(() => {
    ...
    Highcharts.chart(chartContainer1Ref.current, Highcharts.merge(modernTheme, {
      ...
      title: { text: 'Évolution Temporelle des Prix (Modèle décisionnel)' },
      xAxis: {
  ...
  useEffect(() => {
    ...
    paginatedData.slice(0, 15).forEach((item) => {
      ...
      series.push({
        name: `${item.product} [${item.grade}] - Décisionnel`,
        visible: isGradeA,
        dashStyle: 'Solid',
      ...
      series.push({
        name: `${item.product} [${item.grade}] - Théorique`,
        visible: isGradeA,
        dashStyle: 'ShortDash',
    ...
    Highcharts.chart(chartContainer2Ref.current, Highcharts.merge(modernTheme, {
      ...
      title: { text: 'Comparaison Temporelle : Décisionnel vs Théorique' },
      subtitle: { text: 'Affichage initial sur le Grade A' },
  ...
  return (
    <div className="max-w-[1600px] mx-auto p-6 space-y-8">
      ...
      <header className="flex flex-col md:flex-row md:items-center md:justify-between border-b pb-4 border-slate-200">
        <div>
          <h1 className="text-2xl font-bold text-slate-900">Projections Argus (+3 à +36 Mois)</h1>
          <p className="text-sm text-slate-500">Comparaison entre Données Décisionnelles (Réelles + Projections) et Théorie</p>
        </div>
      </header>
      ...
      <section className="bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden space-y-4">
        ...
        <div className="bg-slate-800 text-white px-6 py-3 flex justify-between items-center">
          <h2 className="font-semibold text-lg">Projections théorique</h2>
        </div>
        <div className="overflow-x-auto">
          <table className="w-full text-left text-xs whitespace-nowrap">
            <thead className="bg-slate-50 text-slate-600 border-b">
